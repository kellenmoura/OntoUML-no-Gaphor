...
#Put here this type of stereotypeKind can be head of StereotypeQuality, StereotypeRelator and StereotypeRole.
#Configure to be able to connect only to the 3 in the inheritance relationship

@Connector.register(StereotypeKind, HeritageItem)
class HeritageConnect(RelationshipConnect):
    ...
    def testconnect():
        if handle == StereotypeQuality:
            item = StereotypeQuality
        elif handle == StereotypeRelator:
            item = StereotypeRelator
        elif handle == StereotypeRole:
            item = StereotypeRole

        return allow(StereotypeKind, item)
